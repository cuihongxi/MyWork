...
void NRF_DM();
bool JugeDM();
void RXD_CallBack(Nrf24l01_PRXStr* prx) ;   // ÆÕÍ¨Ä£Ê½½ÓÊÕÍê³É»Øµôº¯Êý
void DMRXD_CallBack(Nrf24l01_PRXStr* prx);	// DMÄ£Ê½½ÓÊÕÍê³É»Øµ÷º¯Êý
...
//³É¹¦×´Ì¬£ºLEDÉÁË¸6´Î£¬·äÃùÆ÷Í¬²½
void StateSuccess();
//DMÄ£Ê½½ÓÊÕÍê³É»Øµ÷º¯Êý
void DMRXD_CallBack(Nrf24l01_PRXStr* prx)
{
	prx->rxlen = NRF24L01_GetRXLen();	
	NRF24L01_Read_Buf(RD_RX_PLOAD,prx->rxbuf,prx->rxlen);	//¶ÁÈ¡Êý¾Ý   	
	NRF24L01_Write_Reg(NRF_WRITE_REG+STATUS,(1 << STATUS_BIT_IRT_RXD)); 	// Çå³ýRX_DSÖÐ¶Ï±êÖ¾
