...
#define			REUSE_TIMES			40			// 发送模式，发送不成功时重发的次数,40*60ms
/**
 * IRQ中断功能配置
 */
#define		IRQ_EXTI_SET()		MYGPIO_EXTI_SET(NRF24L01_IRQ_PIN,NRF_IRQ_Fun,GPIO_PIN_INTR_NEGEDGE)



/*函数*/
/*发射模式*/
void InitNRF_AutoAck_PTX(Nrf24l01_PTXStr* ptx,u8* rxbuf,u8 rxlen,u8 pip,u8 rf_ch,u8* addr);	// 初始化发射模式
...
/*接收模式*/
u8 InitNRF_AutoAck_PRX(Nrf24l01_PRXStr* prx,u8* rxbuf,u8* txbuf,u8 txlen,u8 pip,u8 rf_ch,u8* addr);	// 初始化接收模式
...
void NRF_IRQ_Fun(Nrf24l01_PRXStr* prx);				// 接收模式中断服务函数回调
